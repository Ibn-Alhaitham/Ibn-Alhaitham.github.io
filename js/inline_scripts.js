                ...
                $.getScript("/static/js/words.js?num=" + Math.floor(Math.random() * 100000), function () {
                 Lang = PortalLang;
                    $.getScript("/static/js/index.js?num=" + Math.floor(Math.random() * 100000), function () {
                          //     $.getScript("/static/js/PortalStudent.js?num=" + Math.floor(Math.random() * 100000), function () {
...
//                            PortalLang = 'A';
                                ...
                                $('#portal-nav li').click(function () {
                                    $('#portal-nav li').removeClass('current');
...
//                else if (item=="StudentProgress") viewStudentProgress(type);
    ...
    function GetPassData() {
        var NewPassword = ((Lang == "A") ? "كلمة المرورالجديدة  " : "New Password :");
        var OldPassword = ((Lang == "A") ? "كلمة المرورالحاليه  " : "Current Password :");
        var RePassword = ((Lang == "A") ? "اعادة كلمة المرور  " : "Retype password :");

        // patternReg=`^(|[^'"\\\\])*$`
        pattern = "[^'\x22]+"
        PassData = "";
        PassData += "<table id='ChangePassTbl' dir=" + getName("rtl|ltr", PortalLang) + ">";
    ...
    function showChangePasswordAutomatic() {
        ...
        $.ajax({
            ...
            data: {
                ...
                param1: "check_stu_password_to_change",
            },
            success: function (d) {
                ...
                if (data) {
                    if (data.show_dialog) {
                        ...
                        //  Dict["overlay"] = true;
                        CreatePrivateDialogPassword2(Dict);
                    }
    ...
    function CreatePrivateDialogPassword2(Dict) {
        var str = Dict['str']
        ...
        $('<Div id="DivChangePass" dir="' + getName("rtl|ltr", PortalLang) + '">' + str + '</Div>').dialog({
            modal: true,
            autoOpen: true,
            title: Title,
            closeOnEscape: false,
    ...
    function clickOkonChangePassword2() {
        var Data = {};
                    ...
                    if (data) {
                        if ($("#PassTxt").val() == $("#RePassTxt").val()) {
                            ...
                            if (strcomplex == 'true') {
                                //change password
                                var Data = {};
                                Data["UserPassword"] = $("#PassTxt").val().replace(/\\/g, "\\\\");
    ...
    function drawStudentData() {
        ...
        // ========== MAIN (ثابت - مش بيتغير حسب اللغة) ==========
        var strMain = '    <SECTION role="main" id="main" style="margin-' + getName('right|left') + ': 271px;margin-' + getName('left|right') + ': 0px;">\n' +
            '        <div id="divContents" style="width:100%; padding:15px; box-sizing:border-box;"></div>\n' +
            '    </SECTION>\n';

        // ========== NAVIGATION (بيتغير حسب اللغة) ==========
       var strNav = '    <SECTION role="navigation" id="navigation" >\n' +
    '        <HEADER style="margin-' + getName("right|left", PortalLang) + ':20px; padding: 15px 0 10px 0; border-bottom: 1px solid #e8eef3;">\n' +
    '            <H1 style="color:rgb(15 19 160);font-weight: bold; margin: 0;">\n' +
    '                <font size="4">\n' +
    '                    <p id="logo" class="university-name" style="margin: 0; font-size: 17px;"></p>\n' +
    '                    <em style="font-size:11px; color: #5a7a9a; font-style: normal;">' + ((PortalLang == "A") ? "نظام ابن الهيثم لإدارة شئون الطلاب" : "Ibn al-Hitham System SIS") + '</em>\n' +
    '                </font>\n' +
    '            </H1>\n' +
    '        </HEADER>\n' +
    '        <SECTION id="user-info" style="padding: 12px 15px; margin: 0 15px 10px 15px; background: linear-gradient(135deg, #f5f9fc 0%, #e8f0f5 100%); border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); display: flex; align-items: center; gap: 12px;">\n';

        var strImage = '            <IMG alt="User Avatar" src="/static/images/man.gif" id="UsrImg" style="width: 52px; height: 52px; border-radius: 50%; border: 2px solid #2c7ab1; object-fit: cover; flex-shrink: 0; background: #fff;">\n';

        var strInfo = '            <DIV style="flex: 1; min-width: 0;">\n' +
    '                <DIV id="stdLogName" style="font-size: 14px; font-weight: 600; color: #1a3a5c;">' + ((PortalLang == "A") ? "الطالب" : "Student") + '</DIV>\n' +
    '                <DIV style="font-size: 12px; color: #5a7a9a; margin-top: 2px;">' + ((PortalLang == "A") ? "البرنامج : " : "Program : ") + '<span id="stdProgName" style="font-weight: 500; color: #1a3a5c;"></span></DIV>\n' +
    '                <DIV style="display: flex; gap: 6px; margin-top: 8px; flex-wrap: wrap; align-items: center;">\n' +
    '                    <A title="Change Language" class="user-action" href="#" id="changePortalLang" style="font-size: 11px; color: #2c7ab1; text-decoration: none; padding: 3px 12px; background: rgba(44,122,177,0.08); border-radius: 14px; transition: all 0.2s; display: inline-block;">' + ((PortalLang == "A") ? "تغير اللغه" : "Change Language") + '</A>\n' +
    '                    <A title="Change Password" class="user-action" href="#" id="ChangePass" style="font-size: 11px; color: #2c7ab1; text-decoration: none; padding: 3px 12px; background: rgba(44,122,177,0.08); border-radius: 14px; transition: all 0.2s; display: inline-block;">' + ((PortalLang == "A") ? "تغير كلمة المرور" : "Change Password") + '</A>\n' +
    '                    <A title="Log out" class="user-action" href="#" id="LogOutImg" >' + ((PortalLang == "A") ? "خروج" : "Logout") + '</A>\n' +
    '                </DIV>\n' +
    '            </DIV>\n';

        // ترتيب المحتوى حسب اللغة
        if (PortalLang == "A") {
            strNav += strInfo;
            strNav += strImage;
        } else {
        ...
        strNav += '        </SECTION>\n' +
            '        <NAV id="portal-nav" style="margin-' + getName("right|left", PortalLang) + ': 20px">\n' +
            '            <UL id="portal_menu" style="direction:' + getName("rtl|ltr", PortalLang) + '; text-align:' + getName('right|left', PortalLang) + ';">\n' +
            '            </UL>\n' +
            '        </NAV>\n' +
            '    </SECTION>\n';

        // ========== ترتيب الأقسام حسب اللغة ==========
        if (PortalLang == "A") {
            str += strMain;   // main أولاً (يمين)
            str += strNav;    // navigation ثانياً (يسار)
        } else {
            str += strNav;    // navigation أولاً (يسار)
            str += strMain;   // main ثانياً (يمين)
        }
        ...
        $("#ChangePass").click(function () {
            ...
            $('<Div id="DivChangePass" dir="' + getName("rtl|ltr", PortalLang) + '">'
                + "<table id='ChangePassTbl' dir='" + getName("rtl|ltr", PortalLang) + "'>"
    ...
    function timeTable(state=""){

  var GlobalName = "studytable";
    var str = "  <div id='acceptanceRuleDiv'><table style='padding-top: 15px;direction:" + getName("rtl|ltr") + ";text-align:" + getName("right|left", PortalLang) + "' >";
    str += "      <tr height=30px>";
    str += "          <td>";
  ...
  //  $("#FormStudentPersonalData").hide();
    ...
    $("#divContents").html(str
       ...
       // + '</div><div id="schdl" style="padding:0.1em ; border:1px solid silver; border-collapse: collapse;width:100%;dir=' + getName("rtl:ltr", PortalLang) + '" >  </div>');
            + '</div><div id="schdl" style="padding:0.1em ; width:100%;dir=' + getName("rtl:ltr", PortalLang) + '" >  </div>');
    ...
    $.ajax({
        ...
        success: function(d) {
            ...
            var html = render_basic_student(PortalLang, data);
              $("#divContents").html(html)
            // $("#FormStudentPersonalData").html(html).show();
            // $("#divContents").hide();
        }
    });
}
        ...
        $.ajax({
            ...
            success: function (d) {
                ...
                if (data) {
                    ...
                    //  if (stunationality == "1." && $("#portal_menu").find('#militarystatus').length == 0 ) {
                    //  $("#portal_menu").append('<LI><A title="" style="cursor: pointer;" class="dashboard no-submenu" id="militarystatus">' + getName("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|&nbsp;") + getName("نتيجة التربيه العسكرية|Military Course Status") + '</A></LI>');
...
//                });
    ...
    function render_print_button(lang, buttonName) {
        var align = (lang == "A") ? "right" : "left";
        var floatDir = (lang == "A") ? "left" : "right";

        var html = `
            <div style="padding: 0px 45px 10px 45px; text-align:${align === 'right' ? 'left' : 'right'};">
                <span id="btnPrint${buttonName}" style="float:${floatDir};">
                    ${DrawSystemButton ? DrawSystemButton("Print", buttonName) : ''}
                </span>
            </div>
        `;
                ...
                if (data.length > 0) {
                    ...
                    for (var item in data) {
                                ...
                                if (item.item_id == "mailTD") {
                                    ...
                                    menu_str += "<A title='' class='" + item.item_class + "' href='#' id='" + item.item_id + "' order='" + item.item_order + "'>";
                                    menu_str += "<span class='menu-text'>" + getName(item.item_title, PortalLang) + "</span>";
                                    menu_str += "<span class='menu-icon' data-icon='" + item.item_id + "'></span>";
                                    menu_str += "</A>";
                                    menu_str += "<ul>";
                                    menu_str += "<li><a href='#' id='newMessages' action='create'>";
                                    menu_str += "<span class='menu-text'>" + getName("رسالة جديدة|New Message", PortalLang) + "</span>";
                                    menu_str += "<font id='newMessage'></font></a></li>";
                                    menu_str += "<li><a href='#' id='indoxMessages' action='Indox' class='current'>";
                                    menu_str += "<span class='menu-text'>" + getName("الوارد|Inbox", PortalLang) + "</span>";
                                    menu_str += "<font id='urmCount'></font></a></li>";
                                    menu_str += "<li><a href='#' id='outboxMessages' action='Sent'>";
                                    menu_str += "<span class='menu-text'>" + getName("الصادر|Outbox", PortalLang) + "</span>";
                                    menu_str += "<font id='sentMessages'></font></a></li>";
                                    menu_str += "</ul> </LI>";
                                } else {

                                    menu_str += "<LI style='text-align:" + getName("right|left", PortalLang) + "' >";
                                    menu_str += "<A title='' class='" + item.item_class + "' href='#' order='" + item.item_order + "' id='" + item.item_id + "'>";
                                     menu_str += "<span class='menu-icon' >"+getMenuIcon(item.item_id)+"</span>";
                                    menu_str += "<span class='menu-text'>" +getName(item.item_title, PortalLang)  + "</span>";
                                    menu_str += "</A>";
                                    menu_str += " </LI>";
                                }
...
//                });
                    ...
                    if (Access && Access["access"]) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                var printButtonHtml = render_print_button(PortalLang,GlobalName)
                                retVal.push(printButtonHtml);
                                var tableHtml =   render_exam_seats_html(PortalLang,Data)
                                retVal.push(tableHtml)
                ...
                function warnData(Dict, divID = 'divContents') {
                 ...
                 function loadWarningData(warningType, flagGPAWarning, divID, callback) {
                        var Dict = {
                            'warningType': warningType,
                            'flagGPAWarning': flagGPAWarning,
                            'warningAbsence': (warningType == "2")
                        };
                            ...
                            beforeSend: function() {
                                $('#' + divID).html(`
                                    <div class="loading-message">
                                        <img src="/static/images/LoadingProgressBar.gif">
                                        <br>${getName("جاري تحميل البيانات...|Loading data...", PortalLang)}
                                    </div>
                                `);
                            },
                            success: function(res) {
                            ...
                            error: function() {
                                $('#' + divID).html(`
                                    <div class="error-message">
                                        ${getName("حدث خطأ أثناء تحميل البيانات|Error loading data", PortalLang)}
                                    </div>
                                `);
                            }
                        });
                ...
                function warnDataAcademicWarning(Dict) {
                       var html = render_warning_options_html(PortalLang);
                       $('#divContents').html(html);
                           var defaultWarningType = '1';
                            var flagGPAWarning = Dict['flagGPAWarning'] || true;

                            loadWarningData(defaultWarningType, flagGPAWarning, 'warnDiv', function(result, dict) {
                                var reportHtml = render_warning_data_html(PortalLang, result, dict);
                                $('#warnDiv').html(reportHtml);
                            });
                            $("input[name='warningType']").change(function() {
                                    var warningType = $(this).val();
                                    var flagGPA = (warningType == '1');
                                    var dictData = {
                                        'warningType': warningType,
                                        'flagGPAWarning': flagGPA,
                                        'warningAbsence': (warningType == '2')
                                    };

                                    loadWarningData(warningType, flagGPA, 'warnDiv', function(result, dictData) {
                                        var reportHtml = render_warning_data_html(PortalLang, result, dictData);
                                        $('#warnDiv').html(reportHtml);
                                    });
                                });
...
// function evaluateCondition(selectedCount, operator, desireNumberRegister) {
//
//     const operators = {
//         '>':  (a, b) => a > b,
//         '<':  (a, b) => a < b,
...
//     };
//
//     return operators[operator]?.(selectedCount, desireNumberRegister) ?? false;
// }

function fetchPrograms(progType, callback) {
    var DictProg = {
        'ProgType': progType,
        'isStd': true
    };
    ...
    $.ajax({
        ...
        data: {
            ...
            param2: JSON.stringify(DictProg)
        },
        success: function(d) {
            var data = jQuery.parseJSON(d);
            if (callback && typeof callback === 'function') {
...
function saveDesires(desiresArray, callback) {
    var Data = { 'Desirables': desiresArray };
...
function getDesires() {
    var html = renderDesiresPageHtml(PortalLang);
    $("#divContents").html(html);
    $("#divContents").show();

    $('input[type=radio][name=ProgType][value="0"]').attr('checked', true);

    fetchPrograms('0', function(data) {

        var html = renderPrograms(PortalLang,data);
        // ====== لو في رسالة خطأ ======

           if (html === "") {
                $("#desirediv").html('').hide();
                $(".desires-radio-group").addClass('hidden-radio');
                $("#desiresNoteContainer").show();
                return;
            }
          $(".desires-radio-group").removeClass('hidden-radio');

            $("#desirediv").html(html).show();
...
function bindDesiresEvents() {
    // تغيير نوع البرنامج
    $("input[name='ProgType']").unbind('change');
    $("input[name='ProgType']").change(function() {

        var progType = $(this).val();
        fetchPrograms(progType, function(data) {
           var html = renderPrograms(PortalLang,data);
            $("#desirediv").html(html);
              initMultiSelect();
            repairDesiresData();
    ...
    // زر الطباعة
    $("#PrintProgDsr").die('click');
    $("#PrintProgDsr").live('click', function(e) {

        e.preventDefault();
        fetchPrintData(function(data) {
            if (data) {
                printDesiresReport(data);
            }
        });
        return false;
    });

      $("#SaveDesires").die('click');
    $("#SaveDesires").live('click', function(e) {
        ...
        var desArr = [];
        $('#desirebales').find('.ui-sortable').each(function() {
        ...
        if (desArr.length === 0) {
            showDesiresNote(getName("الرجاء اختيار الرغبات|Please select desires"), 'warning');
            return false;
        }

        saveDesires(desArr, function(result) {
            if (result && result.msg == 'sucess') {
                showDesiresNote(getName('تم الحفظ بنجاح|Data Saved Successfully'), 'info');
                $("#SaveDesires").hide();
                // إعادة تحميل البيانات
                var progType = $('input[name=ProgType]:checked').val();
                fetchPrograms(progType, function(data) {
                    renderPrograms(data);
                    repairDesiresData();
                });
            } else {
                showDesiresNote(result ? result.msg : getName('حدث خطأ|Error'), 'error');
            }
        });
...
function fetchPrintData(callback) {
    var DictProg = { 'isStd': true };
...
//                 if (
//                   //  (desireNumberRegister == "" && selectedCount == allAvaliableProgram )
//                  //   || (desireNumberRegister != "" && selectedCount  >= parseInt(desireNumberRegister))
//
//                     (evaluateCondition(selectedCount,desireOperator,desireNumberRegister))
//                   ) {
//                     var Data = {};
...
//                             if (d['msg'] == 'sucess') {
//                                 $.unblockUI();
//                                 jAlert(getName('تم الحفظ |Data Saved'))
//                                 $("#SaveDesires").hide();
//                                 GetPrograms()
//                             } else {
//                                 jAlert(d['msg'])
...
//                     } else {
//                         var ar_text=("يجب إختيار عدد رغبات  "+desireOperator  + desireNumberRegister)
//                         var en_text=("you must choose number of desires " + desireOperator  + desireNumberRegister + " programs")
//                         jAlert(getName(ar_text+'|'+en_text));
//                         return false
//                     }
//                 }
//
